...
def encrypt(doctype,name,format):
	from cryptography.fernet import Fernet
	message=doctype+"-%-"+name+"-%-"+format
	key = b'D0QTayELFAn1GOMD9xnZ5CKp8Oq_JAu9n3GJ3KEnKNA='
	fernet = Fernet(key)
...
def decrypt(code):
	...
	key = b'D0QTayELFAn1GOMD9xnZ5CKp8Oq_JAu9n3GJ3KEnKNA='
	fernet = Fernet(key)
	decMessage = str(fernet.decrypt(code.encode()).decode())
	return decMessage.split("-%-")
...
def download(code):
        doctype,name,format = decrypt(code)
        html = frappe.get_print(doctype, name, format, doc=None, no_letterhead=0)
        frappe.local.response.filename = "{name}.pdf".format(name=name.replace(" ", "-").replace("/", "-"))
        frappe.local.response.filecontent = get_pdf(html)
        frappe.local.response.type = "pdf"
