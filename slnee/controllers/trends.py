	...
	if filters.get("group_by"):
		...
		data1 = frappe.db.sql(
			""" select %s from `tab%s` t1, `tab%s Item` t2 %s
					where t2.parent = t1.name and t1.order_type = 'Shopping Cart' and t1.company = %s and %s between %s and %s and
					t1.docstatus = 1 %s %s
					group by %s
				"""
			% (
		...
		for d in range(len(data1)):
			...
			row = frappe.db.sql(
				"""select DISTINCT(%s) from `tab%s` t1, `tab%s Item` t2 %s
						where t2.parent = t1.name  and t1.order_type = 'Shopping Cart' and t1.company = %s and %s between %s and %s
